...
export function CreateAdModal() {
    ...
    useEffect(() => {
        axios('http://localhost:3333/games').then((res: any) => { setGames(res.data) })
    ...
    async function handleCreateAd(event: FormEvent) {
        ...
        console.log(enviar)
        try {
            axios.post(`http://localhost:3333/games/${data.game}/ads`, enviar)
                    ...
                    <form onSubmit={handleCreateAd} className='mt-8 flex flex-col gap-4'>
                        <div className='flex flex-col gap-2'>
                            ...
                            <select
                                name="game"
                                id="game"
                                className='bg-zinc-900 px-4 rounded text-sm placeholder:text-zinc-500 p-4 appearance-none'
                                defaultValue=""
                            >
                                ...
                                <div >
                                    ...
                                    >
                                        ...
                                        <ToggleGroup.Item
                                            value={'6'}
                                            title="Sábado"
                                            className={`w-8 h-8 roundedbg-zinc-900 ${weekDays.includes('6') ? 'bg-violet-500' : 'bg-zinc-900'}`}
                                        >S</ToggleGroup.Item>
                                    </ToggleGroup.Root>
                                </div>
                            </div>
                        ...
                        <label className='mt-2 flex items-center gap-2 text-sm'>
                            <Checkbox.Root
                                ...
                                onCheckedChange={(checked) => { if (checked == true) setUseVoiceChannel(true); else setUseVoiceChannel(false) }}
                            >
                                <Checkbox.Indicator >
